...
- current zone
- RSSI
- NearLink connection / active NearLink peer with link RSSI, peer Wi-Fi RSSI/status, MAC and firmware diagnostics; peer labels are best-effort mappings from observed telemetry (`type 1` observed as Dock and `type 2` as RadioLink adapter on WR342E + WA0900), while unknown peer types remain `Unknown`
- rain delay
- rain remaining
